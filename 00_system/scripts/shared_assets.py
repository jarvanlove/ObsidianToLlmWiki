...
def summarize(report: dict[str, object]) -> dict[str, int]:
    ...
    return {status: statuses.count(status) for status in ("current", "safe_update", "missing", "conflict", "source_missing")}
...
def main() -> None:
    ...
    parser.add_argument("command", choices=["report", "stage", "apply-safe"])
