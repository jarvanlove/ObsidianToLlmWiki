...
def initialize_task(repo_root: Path, task_id: str, title: str, level: str, requested_skill: str = "") -> dict[str, Any]:
    requirements = level_requirements(level)
    if level == "U0":
        return {
            "schema_version": 1,
    ...
    payload = {
        ...
        "skill_governance": {
            "requested_skill": requested_skill.strip(),
            "automatic_skills": ["project-ui-rules", "ui-accessibility-check", "ui-screenshot-check"],
            "explicit_skills": [],
        },
...
def set_stage(repo_root: Path, task_id: str, stage: str, approval_note: str = "") -> dict[str, Any]:
    ...
    if stage == "direction_approved":
        ...
        payload.setdefault("design_authority", {}).update({"approval_note": approval_note.strip(), "approved_at": now_iso()})
...
def main() -> None:
    ...
    parser.add_argument("command", choices=["assess", "init", "set-stage", "approve-rfc", "record-evidence", "check"])
    ...
    try:
        ...
        if args.command == "assess":
            ...
            report = initialize_task(repo_root, args.task_id, args.task, args.level, args.requested_skill)
