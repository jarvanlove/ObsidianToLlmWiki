...
- 项目只需接入一次；以后像平常一样直接说需求，例如“修复登录失败”或“给订单页增加导出”
- 系统只在根因、风险、范围或理解需要你决定时打断；项目记忆由系统自动记录、压缩和按需读取
- `开始工作`、`继续`、`收工`仍然保留，但只用于人工检查、恢复中断或显式收尾
...
日常直接描述正常需求。在已接入项目中，普通修改会自动进入同一套工程治理；只读问题不会创建任务。只有根因不明、范围扩大、高风险操作或关键变化需要你理解时，系统才会停下来问你。`开始工作`、`继续`、`收工`是可选的检查与恢复入口。处理知识时直接要求 Agent 摄入、查找、比较或回写资料；脚本调用和严格检查由 Agent 在内部完成。
...
项目接入一次后，直接说你要完成什么，不需要先念启动口令。系统会在后台恢复可信上下文、记录任务边界、检查变更和验证证据。
...
普通低风险工作会低打扰推进；根因未知、范围漂移、P1/P0 风险、证据不足或关键变化尚未被人理解时才会暂停。

想快速了解项目而不翻 Markdown，直接问：`项目现在怎么样`。系统会用同一份可信投影回答，并在本地生成一屏驾驶舱，展示当前状态、最近变化、待决定、开放风险和下一步。

### 可选的人工控制

- `开始工作`：显式检查接入和当前上下文
- `继续`：恢复中断任务并核对 diff
- `收工`：显式检查验证、控制文件和 Wiki 回写是否闭环
...
日常使用仍然直接描述需求。需要维护旧项目记忆时，Agent 必须先执行只读预演，确认备份和冲突报告后才能应用迁移：
