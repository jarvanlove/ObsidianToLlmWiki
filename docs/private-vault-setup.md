# 私有库接入说明

这份文档只讲一件事：怎么把 `ObsidianToWiki-private` 变成你真正使用的知识库。

## 先说清楚公开仓库和私有库的关系

- `ObsidianToWiki`
  公开脚手架，放规则、模板、系统能力和说明
- `ObsidianToWiki-private`
  私有知识库，放你的真实项目、个人知识、原始资料和沉淀结果

这两个目录不是自动双向同步。公开运行时是代码来源，私有库是知识来源。

当前支持的是：

- 用公开仓库的脚手架去同步私有库
- 不把私有内容同步回公开仓库
- 更新时用托管哈希证明文件是否可安全替换，不能证明时只生成候选和备份

## 私有库第一次应该长什么样

至少要有这些顶层目录和文件：

```text
<your-private-vault>/
├─ 00_system/
├─ 01_inbox/
├─ 10_personal/
├─ 20_projects/
├─ 30_shared/
├─ 40_outputs/
├─ 90_archive/
├─ Home.md
├─ 快速开始.md
├─ 使用手册.md
├─ 会话启动页.md
├─ 标准自然语言话术清单.md
├─ index.md
├─ log.md
├─ AGENTS.md
├─ CLAUDE.md
├─ README.md                     # 中文主文档
└─ README-EN.md                  # English guide
```

这些内容由根安装器自动创建。不要手工复制公开仓库的 `AGENTS.md` / `CLAUDE.md` 到私有库；私有库有独立入口模板。

## 个人知识怎么接

个人知识不是“先建复杂结构”，而是先从最小沉淀开始。

建议你先这样用：

1. 新资料先放 `01_inbox/raw/`
2. 有长期价值的内容整理进 `10_personal/`
3. 跨项目可复用的内容提升到 `30_shared/`
4. 一次问答或分析有长期价值时，先落到 `40_outputs/`

最开始你只需要保证：

- `10_personal/索引.md` 存在
- `30_shared/索引.md` 存在
- `40_outputs/索引.md` 存在

## 项目怎么接

如果你有一个现成项目仓库，要把它接入私有库，你只需要说：

- `帮我把当前项目接入 wiki`

默认情况下，系统会优先使用用户级配置，再找公开仓库同级的 `<公开仓库名>-private`。

接入后会发生两件事：

1. 项目仓库里生成：
   - `wiki.context.json`
   - `AGENTS.md`
   - `CLAUDE.md`
2. 私有库里生成项目主干页：
   - `索引.md`
   - `概览.md`
   - `架构.md`
   - `决策.md`
   - `任务.md`
   - `来源.md`
   - `关系.md`
   - `风险.md`
   - `时间线.md`
   - `project.memory.md`

## 资料怎么接进私有库

把资料摄入某个项目时，你只需要说：

- `把这份资料收进当前项目`

把资料先收进全局知识库，再决定后续归类时，你只需要说：

- `把这份资料收进个人知识库`

原始文件会进入输入层，后续再决定是否沉淀到个人层、项目层、共享层或输出层。

对于 PDF、DOCX、PPTX 和长文本，系统应先生成文档地图和带来源定位的章节页，再生成或更新正式知识页：

- PDF 章节保留页码引用，长章节会明确标记续篇。
- 封面和目录不会伪装成正文章节。
- 抽取为空、疑似扫描件或乱码时会停止生成弱质量衍生页。
- 版本、价格和产品状态等时效性内容必须标明来源时间，实际使用前重新核验。
- 正式综述不是越短或越长越好，而是要能回到章节页和原文证据。

## 私有库和公开仓库怎么同步

这个同步属于维护动作，不属于日常用户操作。

你只需要知道：

- 公开仓库负责脚手架层内容，比如 `00_system/`、`docs/`、根入口文件、`30_shared/prompts/`
- 私有库负责真实项目内容、个人知识和原始资料
- 同步不会覆盖你的私有项目内容、个人知识和原始资料
- 已有托管文件只有在记录哈希与当前内容一致时才会更新
- 冲突的新版本进入 `40_outputs/upgrade-candidates/private-scaffold/`，原文件进入带时间戳的更新备份

普通用户只需说 `更新 ObsidianToWiki`。`sync_private_vault.py` 是更新编排器调用的内部能力，不是日常步骤。
