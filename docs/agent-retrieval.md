...
- `scripts/ai/wiki-search.py` reads `runtime_root` and `wiki_root` from `wiki.context.json`, executes the canonical public runtime, and points it at private Markdown.
...
```text
<obsidiantowiki-public-root>/.venv/bin/python -m pip install -r <obsidiantowiki-public-root>/00_system/requirements-mcp.txt
```
