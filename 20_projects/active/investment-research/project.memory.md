...
- 2026-06-23 已完成 P2-3A-H 本地系统运行与缺口复盘：9 条北方华创资料进入资料库，北方华创公司记录完成校准，25 条当前系统支持的公司财务指标已 confirmed，5 个图表数据集/规格已 verified；已创建独立报告项目 `北方华创完整版投研分析 v0.1`，建立 10 个问题驱动章节，绑定资料、47 条报告指标链接和 5 条图表链接；已完成 AI 草稿辅助、L3 审稿流程、3 条审稿意见、阻塞意见门禁验证和 `review_approved` 状态。P2-3H 复盘确认 v0.1 仍缺正式导出版本、发布拆分、checklist/估值快照绑定，且样板闭环服务当前命中旧 seed 报告而非真实 v0.1。下一步应进入 P2-4/P2-5，先写报告导出交付和真实样板闭环识别执行计划，再决定代码实现。
- 2026-06-23 已新增 `docs/product/02-planning/P2-4_P2-5_报告导出交付与真实样板闭环识别执行计划.md`。计划把第一轮代码范围限定为 `P2-4/P2-5-A = 报告 Markdown/HTML 交付包 + 真实 v0.1 样板闭环识别`，明确不同时做 DOCX、PDF、长图、指标模型扩展和审稿任务化；计划已拆出后端导出测试、Markdown 附录增强、HTML 导出 API、报告详情导出 tab、真实样板选择规则测试、样板闭环实现、前端显示、真实 HTTP smoke、无插件浏览器 QA 和文档/Wiki 收口。
- 2026-06-24 已完成 P2-4/P2-5-A 代码实现：报告 Markdown 导出已补来源、指标、图表、审稿和合规附录；新增可打印 HTML 导出 API 和报告详情页导出按钮；导出会创建 `ReportVersion` 快照；样板闭环 `listed_company_full_research` 已优先命中 `business_use=real_sample_report` 的 `北方华创完整版投研分析 v0.1`，旧 seed 保留 fallback warning；后端测试、ruff、mypy、前端 lint/typecheck、本轮文件 Prettier、真实 HTTP smoke 和无插件 Chrome QA 均通过。下一步不要自动开新代码，应先选择 DOCX/PDF 交付、数据质量治理、图表导出增强、审稿任务化或内容继续打磨中的一个方向并补执行计划。
- 2026-06-24 已完成 P2-6A 数据质量治理与上市公司指标模型扩展：新增 `docs/product/02-planning/P2-6_数据质量治理与上市公司指标模型扩展执行计划.md`，将公司财务指标扩展到 19 项，并补充周期类型/标签、来源页码/行号、口径/公式、调整口径、核验状态和质量备注；新增指标定义 API、质量摘要 API、公司研究页数据质量摘要和默认展开的财务指标维护字段。真实 HTTP 验证登录、指标定义、公司列表和质量摘要通过；Playwright 验证北方华创公司研究页可见数据质量摘要、核心指标覆盖、周期类型、数据核验和缺失必需指标，控制台错误为 0。下一步不自动开新代码，应先选择北方华创缺失指标补录、财务指标 CSV/Excel 导入、指标冲突/口径版本治理、DOCX/PDF/长图交付、图表导出增强或真实样板内容打磨。
- 2026-06-24 已新增 `P2-6A-1_北方华创缺失指标补录与核验执行计划.md`：当前系统北方华创财务指标记录 31 条，覆盖率 33%，已核验指标 0；`total_assets`、`equity_parent`、`rd_expense_amount`、`roe_weighted` 已在 CSV 核验但未按新模型写回系统；`revenue`、`net_profit_parent`、`net_profit_deducted`、`operating_cash_flow`、`rd_expense_ratio` 已在系统但缺核验状态和来源定位；`gross_margin`、`inventory`、`accounts_receivable`、`asset_liability_ratio` 等需从年报抽取或公式计算。下一步推荐先做 `P2-6A-2 北方华创补录核验清单 CSV`，再决定 P2-6C 导入开发。
...
- 若用户继续推进，下一步不能自动开启新 Slice；P1 研究工作台化改造已完成，P2 已进入真实样板报告与产品化推进阶段。第一份真实样板已确认为 `北方华创完整版投研分析 v0.1`，P2-2 资料/指标/图表清单、第一批 CSV 输入、47 项人工核验清单、47 项指标文本核验、官方直链回补、P2-3 系统运行计划、P2-3A-H 本地系统运行与缺口复盘、P2-4/P2-5 执行计划和 P2-4/P2-5-A 代码实现均已完成；下一步应先补具体执行计划，再选择 DOCX/PDF 交付、数据质量治理、图表导出增强、审稿任务化或真实样板内容继续打磨。
