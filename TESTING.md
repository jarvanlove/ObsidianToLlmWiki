...
| Governed scaffold compatibility | run `tests.test_engineering_governance_compat tests.test_engineering_governance_evidence tests.test_project_adapter_upgrade tests.test_project_scaffold_upgrade`; verify scaffold v4 creates the governance guide, local customizations survive, conflicts are staged, v1 receipts and active tasks remain usable, JavaScript UTC `Z` evidence is normalized, malformed timestamps still fail, and newer task/adapter state schemas fail closed |
